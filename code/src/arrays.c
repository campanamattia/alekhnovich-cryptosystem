...
struct mat *rand_mat(int rows, int cols){
    ...
    struct mat *m = calloc(1ULL, sizeof(struct mat));
    if(m == NULL)
        return NULL;
    ...
    m->data[rows - 1][cols/SIZE] = m->data[rows - 1][cols/SIZE] << ((cols % SIZE) - 1);
...
uint64_t *weight_array(int len, int weight){
    ...
    for(int i = 0; i < weight; i++){
        ...
        array[pos/SIZE] = array[pos/SIZE] | (1ULL << (pos % SIZE));
    }
...
struct mat *matrix_transpose(struct mat *m){
    ...
    struct mat *t = calloc(1ULL, sizeof(struct mat));
    if(t == NULL)
        return NULL;
...
uint64_t bax(uint64_t *a, uint64_t *b){
    uint64_t res = 0;
    ...
    return count_ones(res); //simplified bit-wise XOR
}
...
struct mat *matrix_mul(struct mat *a, struct mat *b){
    ...
    struct mat *c = calloc(1ULL, sizeof(struct mat));
    if(c == NULL)
        return NULL;
    ...
    for(int i = 0; i < a->rows; i++)
        for(int j = 0; j < b->cols; j++)
            c->data[i][j / SIZE] |= shift_bit(bax(a->data[i], bt->data[j]), j % SIZE);
...
struct mat *matrix_sum(struct mat *a, struct mat *b){
    ...
    struct mat *res = calloc(1ULL, sizeof(struct mat));
    if(res == NULL)
        return NULL;
