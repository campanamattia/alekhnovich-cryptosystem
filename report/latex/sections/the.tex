...
\comm{non usare sottosezioni, usa l'ambiente definition di amsthm per dare definizioni, 
lasciando gli esempi fuori dalla definizione}
\subsection{Matrices Sum}
\comm{definisci prima una matrice con i suoi elementi, la notazione $a_ij$ è flottante}
...
integers \comm{denota come $\bmod 2$} mod 2, i.e., $\{0, 1\}$. 
...
\comm{sembra un po' sbilanciato definire concetti più semplici come somma
e prodotto di matrici e glissare su quelli relativi agli spazi vettoriali. Riporta
le proprietà definitorie di campo e spazio vettoriale.}
...
\subsection{Code}
The code it can seen as a function \comm{no, il codice non è una funzione, è l'insieme
dei vettori immagine} that given a word $x$ encode that word over 
...
\comm{come sopra per gli spazi vettoriali, è il caso di dare definizioni proprie
anche qui}
...
For instance, consider two strings: $101010$ and $111011$. Their Hamming 
...
Various algorithms \comm{non si tratta tanto degli algoritmi, quanto del fatto
che alcuni codici ammettono algoritmi di decodifica. Quelli che menzioni
nel seguito sono poi infatti esempi di codici} have been implemented over 
time, ranging from the most obvious and simple to the most effective and 
complex, including:
\begin{itemize}
    \item Repetition Code: a coding scheme that repeats blocks of bits n \comm{le variabili, sempre in math-mode, ovvero $n$, non n} times,
    ...
    family.\comm{questo punto è proprio sbagliato: ``error correcting code'' non denota
    nulla in particolare. A margine, il repetition code, è anch'esso un codice, quindi
    i due punti non sono separati suggerisco di indicare e descrivere, per le
    famiglie di codici, il repetition, l'Hamming e l'extended Hamming} 
...
In the context where the code $C$ \comm{$\mathcal{C}$} 
...
into rows within a generating \comm{generator} matrix $G$. 
This matrix serves as a representation  of the code itself, simplifying the 
transformation of data vectors \comm{information words} into code words through 
...
the kernel of a linear map. The kernel of a matrix, also known as the null space, refers to the kernel of the linear map defined by the matrix.\comm{decisamente, questa non è una footnote, è contenuto da mettere come definizioni nella sezione di preliminari}} being $C$. The code that $H$ can generate is termed the dual code of $C$. When the $G$ matrix takes the form $[\mathbb{I}_k | P]$, it is considered to be in standard form, and consequently, the $H$ matrix appears as $[-P^T | \mathbb{I}_{n-k}]$\comm{questo fatto sembra un po' calato dal cielo, espandi
il calcolo relativo ad eso}.
